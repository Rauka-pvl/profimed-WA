                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        <input type="text" name="search" class="form-control"
                            placeholder="Поиск по пациенту, врачу, телефону..." value="{{ request('search') }}">
                    </div>
            ...
            @if ($appointments->count() > 0)
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        @php
                            function sortLink($label, $field, $currentField, $currentDir)
                            {
                                ...
                                $icon =
                                    $currentField === $field
                                        ? ($currentDir === 'asc'
                                            ...
                                            : 'bi-caret-down-fill')
                                        : 'bi-arrow-down-up';
                                $query = array_merge(request()->query(), ['sort_by' => $field, 'sort_dir' => $dir]);
                    ...
                    {$label} <i class='bi {$icon}'></i>
                        ...
                        <thead class="table-light">
                            <tr>
                                <th>{!! sortLink('Дата', 'date', $sortField, $sortDir) !!}</th>
                                <th>{!! sortLink('Время', 'time', $sortField, $sortDir) !!}</th>
                                <th>{!! sortLink('Пациент', 'patient', $sortField, $sortDir) !!}</th>
                                <th>Телефон</th>
                                <th>{!! sortLink('Врач', 'doctor', $sortField, $sortDir) !!}</th>
                                <th>Кабинет</th>
                                <th>{!! sortLink('Статус', 'status', $sortField, $sortDir) !!}</th>
                                <th>Напоминания</th>
                                <th>Действия</th>
                            </tr>
                        </thead>
